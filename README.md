...
| **Isolation** | Regional remoteness, using **terrain-aware** distance: a distance-weighted count of nearby population plus the effective distance to the nearest city of ≥50k, where straight-line distance is inflated by the climb along the path. A metro behind a mountain range counts as far (a town over the San Gabriels from LA scores isolated), while a flat suburb the same distance out does not. | Census places + 2020 decennial population + Copernicus DEM GLO-90 |
